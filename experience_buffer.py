...
class ExperienceBuffer:
    """ Class representing memory gathered during plays, collected for training."""
    def __init__(self, buffer_size, alpha=0.8, gamma=0.95):
    ...
    def add_trajectory(self, trajectory, algorithm):
        ...
        for i in range(len(trajectory)-1):
            ...
            nxt = trajectory[i+1]
            q_value = np.array([algorithm.q_value(current[2], nxt[2], nxt[3],
                                                  self.alpha, self.gamma, nxt[4])])
            state = current[0]
            action = np.zeros(9)
            action[current[1]] = 1
